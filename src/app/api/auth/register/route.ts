...
export async function POST(request: NextRequest) {
  ...
  if(newUser.invitation){
    ...
    if(!invitation){
      return NextResponse.json({ message: "La invitación no existe" }, { status: 403 })
    }
    if(invitation.expiresAt.getTime() < Date.now()){
      return NextResponse.json({ message: "La invitación ha expirado" }, { status: 410 })
    }
    if(invitation.usedAt){
      return NextResponse.json({ message: "Esta invitación ha sido utilizada" }, { status: 410 })
    }
    if(invitation.email !== newUser.email){
      return NextResponse.json({ message: "Email no autorizado" }, { status: 403 })
    }
  }
  ...
  if (user) {
    ...
    if(invitation && tenant){
      ...
      if(!tenantUser){
        return NextResponse.json({ message: "Ocurrió un error al asignar el usuario a su tenant", user }, { status: 500 });    
      }
